      ...
      <Table background={true}>
        <Table.Header className="bg-white">
          <Table.HeaderColumn
            ...
            aria-sort={sortColumn === 'recipient.personnumber' ? sortOrder : 'none'}
          >
            {recipients.length > 1 ? (
              <Table.SortButton
                isActive={sortColumn === 'recipient.personnumber'}
                sortOrder={sortOrder}
                onClick={() => handleSorting('recipient.personnumber')}
              >
                Personnummer
              </Table.SortButton>
            ) : (
          ...
          <Table.HeaderColumn
            className="bg-background-color-mixin-1"
            aria-sort={sortColumn === 'address.lastname' ? sortOrder : 'none'}
          >
            {recipients.length > 1 ? (
              <Table.SortButton
                isActive={sortColumn === 'address.lastname'}
                sortOrder={sortOrder}
                onClick={() => handleSorting('address.lastname')}
              >
                Namn
              </Table.SortButton>
            ) : (
          ...
          <Table.HeaderColumn
            className="bg-background-color-mixin-1"
            aria-sort={sortColumn === 'address.addresses.0.address' ? sortOrder : 'none'}
          >
            {recipients.length > 1 ? (
              <Table.SortButton
                isActive={sortColumn === 'address.addresses.0.address'}
                sortOrder={sortOrder}
                onClick={() => handleSorting('address.addresses.0.address')}
              >
                Adress
              </Table.SortButton>
            ) : (
          ...
          <Table.HeaderColumn
            className="bg-background-color-mixin-1"
            aria-sort={sortColumn === 'error' ? sortOrder : 'none'}
          >
            {recipients.length > 1 ? (
              <Table.SortButton
                isActive={sortColumn === 'error'}
                sortOrder={sortOrder}
                onClick={() => handleSorting('error')}
              >
                Status
              </Table.SortButton>
            ) : (
            ...
            .map((d, idx) => (
              <Table.Row key={`row-${idx}`}>
                ...
                <Table.Column>
                  {
                    <span>
                      {d.address?.lastname && d.address?.givenname
                        ? `${d.address?.lastname}, ${d?.address?.givenname}`
                        : ''}
                    </span>
                  }
                ...
                <Table.Column>
                  {d.address?.addresses?.length > 0 ? (
                    <span>
                      {d?.address?.addresses[0]?.address}
                      {d?.address?.addresses[0]?.addressLetter}, {d?.address?.addresses[0]?.postalCode}{' '}
                      {d?.address?.addresses[0]?.city} {d?.address?.addresses[0]?.country}
                    </span>
                  ) : (
                    <></>
                  )}
          ...
          <Table.Footer>
            <div className="sk-table-bottom-section">
              <div className="sk-table-bottom-section sk-table-pagination-mobile">
                ...
                >
                  {[...Array.from(Array(Math.ceil(totalPages / _pageSize)).keys())].map((page) => (
              ...
              <div className="sk-table-paginationwrapper">
                <Pagination
                  ...
                  pages={Math.ceil(totalPages / _pageSize)}
