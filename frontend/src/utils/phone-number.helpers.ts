...
function isValidMobileSanitized(s: string): boolean {
  ...
  return /^(?:07\d\d{7}|\+467[2-9]\d{7}|00467[2-9]\d{7}|7[2-9]\d{8})$/.test(s);
}
...
function normalizeMobileOrEmpty(s: string): string {
  ...
  if (/^00467\d{8}$/.test(s)) return s.replace(/^0046/, '+46');
  if (/^07\d{8}$/.test(s)) return s.replace(/^0/, '+46');
  if (/^7\d{8}$/.test(s)) return `+46${s}`;
  return '';
}
